...
const NotificationList = () => {
    ...
    return (
        <div className="flex flex-col w-full items-center justify-center">
            ...
            <div className="flex mt-5 py-5 flex-start w-11/12">
                <p className={`md:text-xl text-sm ${isDarkMode ? 'text-white' : 'text-black'}`}>New</p>
            </div>
            {messageList.map((mdata,index) =>
            <div key={index} className="flex w-full flex-col items-center justify-center">
                <NotificationComponent mdata={mdata}/>
            </div>
            )}
            <div className="flex md:mt-5 md:py-5 flex-start w-11/12">
                <p className={`md:text-xl text-sm ${isDarkMode ? 'text-white' : 'text-black'} flex-start`}>Earlier</p>
            </div>
            {messageList.map((mdata,index) =>
            <div key={index} className="flex w-full flex-col items-center justify-center">
                <NotificationComponent mdata={mdata}/>
